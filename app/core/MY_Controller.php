...
class MY_Controller extends CI_Controller {
	...
	public function __construct() {
		...
		$this->twig->addFilter('randfromlist', function($list, $count = false) {
			if (!$list || !is_array($list) || !$count) return false;
			shuffle($list);
		...
		$this->twig->addFilter('sortbykey', function($arr = [], $dir = 'asc') {
			if (!$arr) return [];
			if ($dir == 'asc') ksort($arr);
			if ($dir == 'desc') krsort($arr);
			return $arr;
		});
		...
		$this->twig->addFilter('regroup', function($arr = false, ...$fields) {
			if (!$arr || !$field) return false;
			$newData = []; $str = '';
			foreach ($fields as $field) $str .= "[\$item['".$field."']]";
		...
		$this->twig->addFilter('nlreplace', function($string = false, $replace = false) {
			if (!$string || !$replace) return false;
			$splitTag = explode('><', $replace);
			if (!isset($splitTag[0]) || !isset($splitTag[1])) return $string;
			$startTag = $splitTag[0].'>';
			$endTag = '<'.$splitTag[1];
			if (!$splitStr = array_filter(preg_split("/\r\n|\r|\n/", $string))) return $string;
			$finalStr = '';
			foreach ($splitStr as $item) $finalStr .= $startTag.$item.$endTag;
			return $finalStr;
		});
		...
		*/
		$this->twig->addFilter('parsedown', function($str, $multiline = false) {
			...
			if ($multiline) $text = $this->parsedown->text($str);
			else $text = $this->parsedown->line($str);
			return $text;
		});
		...
		*/
		$this->twig->addFilter('recursive', function($data, $item = false, $childField = false, $wrapTag = '<ul></ul>', $levelLimit = false) {
			...
			$wtgs = $wrapTags[0];
			if (!isset($wtgs[1])) $wtgs[1] = preg_replace('/(\w+)[^>]+/i', '/$1', $wtgs[0]);
			...
			if ($iter) {
				foreach ($iter as $k => $row) {
					...
					$list = preg_replace_callback('/\{(\w+)\}/ui', function($m) use ($row) {
						if (!isset($m[1]) || !isset($row[$m[1]])) return false;
						return $row[$m[1]];
					}, $i);
	...
	protected function display($ops = [], $settings = []) {
		if (!isset($ops['page']) || !$ops['page']) return [];
		...
		if (!is_file('public/views/'.$this->controllerName.'/pages/'.$page)) $options['page'] = 'error';
	...
	*/
		...
		if ($this->upload->do_upload(false, $file)) {
    		...
    		if ($uploadData['is_image'] == 1) { // если загруженный файл - изображение
        		if (isset($thumb_w) || isset($thumb_h)) {
					...
					if (!is_dir($fullPath.$thumbsPath.'/')) mkdir($fullPath.$thumbsPath.'/');
				...
				if (isset($resize_w) || isset($resize_h)) {
					...
					if (isset($resize_w) && $uploadData['image_width'] > $resize_w) $cfgr['width'] = $resize_w;
					if (isset($resize_h) && $uploadData['image_height'] > $resize_h) $cfgr['height'] = $resize_h;
	...
	protected function _removeFile($fileName = null, $path = null, $thumbsPath = false) {
		...
		if (is_file($thumbsPath.$fileName)) unlink($thumbsPath.$fileName);
		if (is_file($fullPath.$fileName)) unlink($fullPath.$fileName);
		return true;
	}
